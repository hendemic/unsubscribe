...
impl SenderHistoryView {
    /// Whether this sender kept mailing after a successful unsubscribe.
    #[must_use]
...
pub fn visible_histories(views: &[SenderHistoryView], filter: &HistoryFilter) -> Vec<usize> {
    let mut visible: Vec<usize> = views
        .iter()
        .enumerate()
        .filter(|(_, view)| !filter.resumed_only || view.is_resumed())
        .filter(|(_, view)| matches_search(view, &filter.search))
        .map(|(index, _)| index)
        .collect();
